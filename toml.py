...
# should be run with exec with an "mg" global

from hypothesis import given, strategies as st, seed, settings, HealthCheck
from string import printable
from math import isnan
from tqdm import tqdm

# code taken from https://github.com/uiri/toml/tree/3f637dba5f68db63d4b30967fedda51c82459471/toml
...
@mg.has_mutant("STR_NO_LEADING_DOT", description="Section names starting with a period throw errors")
@mg.has_mutant("SECTION_NAME_NO_BRACKETS", description="Section names containing brackets throw errors")
@mg.has_mutant("NO_PARSE_ESCAPED_QUOTES", description="Escaped quotes aren't parsed correctly")
            ...
            try:
                while sl[i - k] == '"':
                    ...
                    if k == 3:
                        tripquote = True
                        break
                if mg.active_mutant("NO_PARSE_ESCAPED_QUOTES") or k == 1 or (k == 3 and tripquote):
                    while sl[i - k] == '\\':
                        oddbackslash = not oddbackslash
                        k += 1
            except IndexError:
        ...
        if line[0] == '[':
            ...
            if len(line) == 1:
                raise TomlDecodeError("Opening key group bracket on line by "
                                      "itself.", original, pos)
            if mg.not_mutant("SECTION_NAME_NO_BRACKETS"):
                if line[1] == '[':
                    arrayoftables = True
                    line = line[2:]
                    splitstr = ']]'
            ...
            i = 0
            while i < len(groups):
                stripped_group = groups[i].strip()
                if len(stripped_group) > 0 and (stripped_group[0] == '"' or stripped_group[0] == "'"):
                    groupstr = groups[i]
                    j = i + 1
                    while len(decoder._get_split_on_quotes(groupstr)) < 3 or (mg.not_mutant("STR_NO_LEADING_DOT") and len(groupstr) == 1):
                        j += 1
                        if j > len(groups) + 2:
                    ...
                    parsed_str = decoder.load_value(groupstr)
                    assert parsed_str[1] == "str"
                    groups[i] = parsed_str[0]
                    groups[i + 1:j] = []
                else:
                    groups[i] = stripped_group
            ...
            for i in _range(len(groups)):
                ...
                try:
                    currentlevel[group]
                    # print("pepega", implicitgroups, repr(group))
                    if i == len(groups) - 1:
...
@mg.has_mutant("FLOAT_DATETIME_CONFUSION", description="Sometimes floats were incorrectly parsed as datetimes")
def _load_date(val):
    microsecond = 0
    tz = None
    try:
        ...
        tz = None
    if mg.not_mutant("FLOAT_DATETIME_CONFUSION") and "-" not in val[1:]:
        return None
    try:
        if len(val) == 10:
            d = datetime.date(
...
_escapedchars_to_escape = dict(zip(_escapedchars, _escapes))
...
def _unescape(v):
    ...
    while i < len(v):
        if backslash:
            backslash = False
            if v[i] in _escape_to_escapedchars:
                ...
                assert i + 4 < len(v)
                esc = v[i + 1:i + 5]
                v = v[:i - 1] + chr(int(esc, 16)) + v[i + 5:]
            elif v[i] == 'U':
                assert i + 8 < len(v)
                esc = v[i + 1:i + 9]
                v = v[:i - 1] + chr(int(esc, 16)) + v[i + 9:]
            else:
...
class TomlDecoder(object):
    ...
    @mg.has_mutant("NO_QUOTES_IN_SECTION_NAME", description="Quotes in section names raise errors.")
    def _get_split_on_quotes(self, line):
        if mg.not_mutant("NO_QUOTES_IN_SECTION_NAME"):
            doublequotesplits = []
            i = 0
            while i < len(line):
                try:
                    ...
                    while j > 0:
                        ...
                        if backslashes % 2 == 0:
                            break
                        j = line.index('"', j + 1)
                    doublequotesplits.append(line[i:j])
                    i = j + 1
                except ValueError:
                    doublequotesplits.append(line[i:])
                    break
            else:
                doublequotesplits.append('')
            return doublequotesplits
        else:
        ...
        quoted = False
        quotesplits = []
        if mg.not_mutant("SECTION_NAME_NO_BRACKETS") and len(doublequotesplits) > 1 and "'" in doublequotesplits[0]:
    ...
    @mg.has_mutant("NO_UNESCAPE_KEYS", description="Quoted keys aren't unescaped")
    @mg.has_mutant("STRINGS_SPLIT_EARLY", description="Equal signs in strings are parsed as outside the string")
    @mg.has_mutant("DOT_DOESNT_UNESCAPE", description="Dots in key names cause key name to not unescape")
    def load_line(self, line, currentlevel, multikey, multibackslash):
        i = 1
        if mg.not_mutant("STRINGS_SPLIT_EARLY"):
        ...
        if '.' in pair[0]:
            if '"' in pair[0] or "'" in pair[0]:
                ...
                levels = pair[0].split('.')
            if mg.not_mutant("DOT_DOESNT_UNESCAPE"):
                levels = [_unescape(x) for x in levels]
            while levels[-1] == "":
                levels = levels[:-1]
            ...
            pair[0] = levels[-1]
                ...
                (pair[0][-1] == pair[0][0]):
            pair[0] = mg.mut("NO_UNESCAPE_KEYS", lambda: _unescape(pair[0][1:-1]), lambda: pair[0][1:-1])
            # print("unescaping", repr(pair[0]))
    ...
    @mg.has_mutant("NO_PARSE_LONE_QUOTE", description="A single escaped double quote in a string will be dropped on decoding")
    def load_value(self, v, strictly_valid=True):
        ...
        if v == 'true':
            ...
            if quotechar == '"':
                ...
                #                                   prefix)
                v = _unescape(v)
            if mg.mut("NO_PARSE_LONE_QUOTE",
                lambda: triplequote,
                lambda: len(v) > 1 and v[1] == quotechar and (len(v) < 3 or v[1] == v[2])):
                v = v[2:-2]
            return (v[1:-1], "str")
        elif v[0] == '[':
    ...
    @mg.has_mutant("NO_QUOTE_COMMA_IN_LIST", description="A double quote followed by a comma in a list throws an error")
    @mg.has_mutant("NO_EMPTY_STRING_IN_LIST", description="An empty string in a list can break parsing")
    def load_array(self, a):
        ...
        if '[' not in a[1:-1] or "" != a[1:-1].split('[')[0].strip():
            ...
            if strarray:
                while b < len(a) - 1:
                    ab = a[b].strip()
                    while (mg.mut("NO_QUOTE_COMMA_IN_LIST", lambda: not self.bounded_string(ab), lambda: ab[-1] != ab[0]) or
                           ((mg.active_mutant("NO_EMPTY_STRING_IN_LIST") or len(ab) > 2) and
                            ab[0] == ab[1] == ab[2] and
                            ab[-2] != ab[0] and
                            ab[-3] != ab[0])):
                        a[b] = a[b] + ',' + a[b + 1]
...
@mg.has_mutant("BAD_CIRCULAR_REF_CHECK", description="Circular ref checking sometimes triggers a false positive")
def dumps(o, encoder=None):
    ...
    outer_objs = [id(o)]
    while sections:
        section_ids = [id(section) for section in mg.mut("BAD_CIRCULAR_REF_CHECK", lambda: sections.values(), lambda: sections)]
...
@mg.has_mutant("STR_DUMP_CANT_HANDLE_ESCAPE", description="Hex escapes are handled incorrectly")
def _dump_str(v):
    if mg.not_mutant("STR_DUMP_CANT_HANDLE_ESCAPE"):
        ...
        for char in v:
            c = ord(char)
            if c >= 0x7f or c <= 0x1f:
                if char in _escapedchars_to_escape:
                    out += "\\" + _escapedchars_to_escape[char]
                    continue
                h = hex(c)[2:]
                if len(h) <= 4:
                    h = '0' * (4 - len(h)) + h
...
@mg.has_mutant("FLOATS_TURN_TO_INTS", description="Floats lose .0 when serialized, causing them to become ints")
def _dump_float(v):
    return (mg.mut("FLOATS_TURN_TO_INTS", lambda: "{}", lambda: "{:g}")).format(v).replace("e+0", "e+").replace("e-0", "e-")
...
class TomlEncoder(object):
    ...
    @mg.has_mutant("NO_DUMP_EMPTY_DICT_IN_LIST", description="An empty dict in a list isn't dumped")
    @mg.has_mutant("NO_ESCAPE_KEYS", description="Keys aren't escaped the same way that values are")
    def dump_sections(self, o, sup):
        ...
        for section in o:
            ...
            if any(x not in charset for x in section):
                if mg.not_mutant("NO_ESCAPE_KEYS"):
                        ...
                        while d:
                            ...
                            for dsec in d:
                                s1, d1 = self.dump_sections(d[dsec], sup +
                                                            qsection + "." +
                                                            dsec)
                                if mg.not_mutant("NO_DUMP_EMPTY_DICT_IN_LIST") or s1:
...
# compare orig and new for equality with a few caveats:
# - dict keys will be cast to strings before comparing
# - non-serializable values like None will be compared with string serialization instead
# - ints and floats will be considered inequal, e.g. 10 != 10.0
# - iterables and lists will be considered equal
def structural_compare(orig, new):
    # print(orig, new)
    if isinstance(orig, dict):
        if not isinstance(new, dict) or set(str(x) for x in orig.keys()) != new.keys():
            return False
        return all(structural_compare(orig[k], new[k]) for k in orig)
    if isinstance(orig, list):
        if not isinstance(new, list) or len(orig) != len(new):
            return False
        return all(structural_compare(a, b) for (a, b) in zip(orig, new))
    for cls in (str, bool, int, float):
        if isinstance(orig, cls):
            if not isinstance(new, cls):
                return False
            return (cls == float and isnan(orig) and isnan(new)) or orig == new
    if is_iterable(orig):
        return isinstance(new, list) and list(orig) == new
    return isinstance(new, str) and str(orig) == new

hard_cap = 3
key_strategy = st.text(min_size=1)
def gen_value_strategies(inner):
    ret = [
        st.booleans(), 
        st.integers(),
        st.floats(),
        st.text(),
        st.dictionaries(key_strategy, inner, max_size=hard_cap)
    ]
    # lists must be homogenous
    ret += [st.lists(x, max_size=hard_cap) for x in ret]
    return ret

inner_strategy = st.deferred(lambda: st.one_of(*gen_value_strategies(inner_strategy)))
# strategy = st.dictionaries(key_strategy, st.one_of(*all_strategies), max_size=3)
testing_strategy = st.dictionaries(key_strategy, inner_strategy, max_size=hard_cap)

def testing_function(x):
    res = structural_compare(x, loads(dumps(x)))
    if not res:
        raise Exception("ono different")
